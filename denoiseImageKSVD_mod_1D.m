function [IOut,output] = denoiseImageKSVD_mod_1D(signal,sigma,K,varargin)
%==========================================================================
...
%         [row,col] = ind2sub(size(signal)-bb+1,selectedBlocks(i));
%         currBlock = signal(1,col:col+bb-1);
%         blkMatrix(:,i) = currBlock(:);
%     end
% else
%     blkMatrix = im2col(signal,[bb,bb],'sliding');
% end
% Generate blocks from the signal
...
for i=1:maxNumBlocksToTrainOn
    ...
    blkMatrix = [blkMatrix signal(1,idxW:(idxW+bb-1))'];
    idx = [idx idxW];
    idxW = idxW + slidingDis;
end
...
% Dicionario inicial DCT. Usar Wavelets para as DPs
% Pn=ceil(sqrt(K));
% DCT=zeros(bb,Pn);
% for k=0:1:Pn-1,
...
%param.initialDictionary = wmpdictionary(bb,'lstcpt',{{'sym4',5},{'sym4',4},{'sym4',2},{'sym4',3},{'sym4',1} });
param.initialDictionary = wmpdictionary(bb);
param.InitializationMethod = 'GivenMatrix';
%param.InitializationMethod = 'DataElements';
param.K = size(param.initialDictionary,2);
if (reduceDC)
    vecOfMeans = mean(blkMatrix);
    blkMatrix_mean = blkMatrix-ones(size(blkMatrix,1),1)*vecOfMeans;
end
...
[Dictionary,output] = KSVD(blkMatrix_mean,param);
output.D = Dictionary;
...
while (prod(floor((size(signal)-bb)/slidingDis)+1)>maxBlocksToConsider)
    slidingDis = slidingDis+1;
end
%[blocks,idx] = my_im2col(signal,[bb,bb],slidingDis);
blocks = blkMatrix;
...
% go with jumps of 100
for jj = 1:100:size(blocks,2)
    if (waitBarOn)
        waitbar(((param.numIteration*size(blocks,2))+jj)/newCounterForWaitBar);
    end
    jumpSize = min(jj+100-1,size(blocks,2));
    if (reduceDC)
    ...
    Coefs = OMP(Dictionary,blocks(:,jj:jumpSize),2);
    if (reduceDC)
...
Weight = zeros(NN1,NN2);
IMout = zeros(NN1,NN2);
%[rows,cols] = ind2sub(size(signal,2)-bb+1,idx);
for i  = 1:length(idx)
    col = idx(i); %row = rows(i);        
    %block =reshape(blocks(:,count),[bb,bb]);
    IMout(1,col:col+bb-1)=IMout(1,col:col+bb-1)+ blocks(:,count)';
    Weight(1,col:col+bb-1)=Weight(1,col:col+bb-1)+ones(1,bb);
    count = count+1;
end;
...
IOut = (signal+0.034*sigma*IMout)./(1+0.034*sigma*Weight);
plot(IOut); 
figure; plot(IMout);
% figure;
% plot(IOut - signal);
